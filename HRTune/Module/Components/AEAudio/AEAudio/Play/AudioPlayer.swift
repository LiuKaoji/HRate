...
@objc public protocol AudioPlayerDelegate: AnyObject {
    ...
    @objc optional func player(_ player: AudioPlayer, didUpdateoutCoverImage image: UIImage)
...
extension AudioPlayer {
    ...
    private func updateNowPlayingInfo() {
        ...
        // 如果 info.outCoverImage 非空，则创建 MPMediaItemArtwork 对象并添加到 nowPlayingInfo 字典中
        let image = self.getoutCoverImage()
        if let outCoverImage = image {
            let artwork = MPMediaItemArtwork(boundsSize: outCoverImage.size) { _ in
                return outCoverImage
            }
            nowPlayingInfo[MPMediaItemPropertyArtwork] = artwork
            delegate?.player?(self, didUpdateoutCoverImage: outCoverImage)
        }
    ...
    // 获取封面图像
     private func getoutCoverImage() -> UIImage? {
         ...
         if let imageURL = bundle.url(forResource: "cover", withExtension: "jpeg") {
