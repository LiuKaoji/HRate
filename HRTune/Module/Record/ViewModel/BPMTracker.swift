...
extension BPMTracker{
            ...
            self.healthStore.startWatchApp(with: configuration) { _, error in
                DispatchQueue.main.async {
                    if let error = error {
                        ...
                        HRToast(message: HealthStoreError.parse(error as NSError), type: .error)
                        handler(error)
                    } else {
    ...
    func toggleRunning(completion: @escaping (Error?) -> Void) {
            if state.value != .running {
                ...
                wcManager.fetchReachableState { [weak self] isReachable in
                    if isReachable {
                        wcManager.send([.workoutStop: true])
                    } else {
                        wcManager.transfer([.workoutStop: true])
                    }
                    self?.workoutData.accept(WorkoutData.defaultWorkout)
                    completion(nil)
                }
            }
        }
